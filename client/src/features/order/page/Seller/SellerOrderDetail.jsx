...
// Shared with SellerOrder.jsx / SellerAnalytics.jsx — worth lifting into one
// file (e.g. sellerUi.js) once a third page needs the same tokens.
...
// The full lifecycle used to plot which steps are "ahead" vs "reached" —
// terminal/side-branch statuses (CANCELLED, RETURNED, EXCHANGED) are handled
// separately since they don't sit on the main line.
...
// ---------------------------------------------------------------------------
const Card = ({ title, description, children, className = "", action }) => {
...
// Animated vertical timeline built from statusHistory. Each dot fades/slides
// in with a stagger, and the connecting line animates its height so the
// path feels like it's being drawn as you read down.
      ...
      {sorted.map((step, i) => {
        ...
        return (
          ...
          >
            {!isLast && (
              <motion.span
                className="absolute left-[15px] top-8 bottom-0 w-px origin-top"
                style={{ backgroundColor: "currentColor" }}
                initial={reduceMotion ? { opacity: 1 } : { scaleY: 0 }}
...
const SellerOrderDetail = () => {
  ...
  const { handelGetSellerOrderDetail } = useSellerOrder();
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await handelGetSellerOrderDetail(orderid);
        setData(res);
      } catch (err) {
        console.error(err);
      } finally {
            ...
            >
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <StatusUpdateDropdown
                  ...
                  onUpdated={(updatedOrder) => setData(updatedOrder)}
                />
              </div>
              ...
              <div className="space-y-4">
                ...
                <Card title="Payment">
                  ...
                  {data.payment?.razorpay?.paymentId && (
                    <div className="pt-3 border-t border-zinc-100 dark:border-white/[0.06] space-y-2">
                      <div>
                        <p className="text-xs text-zinc-500 dark:text-zinc-400">
                          Payment ID
                        </p>
                        <p className="text-xs font-mono text-zinc-700 dark:text-zinc-300 truncate mt-0.5">
                          {data.payment.razorpay.paymentId}
                        </p>
                      </div>
                      <div>
